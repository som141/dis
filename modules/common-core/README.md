...
`common-core`는 별도 실행 앱이 아니라 두 실행 앱이 공유하는 공용 라이브러리 모듈이다. 공용 계약, 재생 코어, Redis/RabbitMQ 인프라, 공통 bootstrap을 제공한다.
...
- event transport: Spring local event 고정
- InMemory 구현: 제거
...
현재 구조에서 공용 코어를 없애면 아래 둘 중 하나가 된다.

1. 공용 코드를 두 앱에 중복 복사
2. 한 앱이 다른 앱 코드를 직접 참조

둘 다 유지보수성이 나쁘기 때문에, 현재 구조에서는 공용 코어를 별도 모듈로 두는 편이 더 명확하다.
