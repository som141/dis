...
지금 [logback-spring.xml](/C:/Users/s0302/OneDrive/바탕%20화면/portpolio/dis/modules/common-core/src/main/resources/logback-spring.xml)은 구조 로그 엔트리 포맷을 제어하는 파일이다. 기존 평문 패턴 로그에서 JSON 구조 로그로 전환하는 핵심 지점이다.
...
3. [modules/common-core/src/main/resources/logback-spring.xml](/C:/Users/s0302/OneDrive/바탕%20화면/portpolio/dis/modules/common-core/src/main/resources/logback-spring.xml)
   - JSON 로그로 전환
...
현재 저장소에는 아래 항목이 이미 반영되어 있다.

- `Prometheus` scrape 설정
- `Loki + Alloy` 로그 수집 경로
- `Grafana` datasource provisioning
- `Discord Bot App Overview` 대시보드
- `Discord Bot Infra Overview` 대시보드
...
다음 단계는 대시보드 고도화와 실제 알림 라우팅 연결이다.

- Discord webhook 실환경 연결
- GitHub Actions / 서버 배포 경로에 Grafana alert env 주입
- Loki 기반 에러 로그 알림
